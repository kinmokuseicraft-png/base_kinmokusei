/**
 * GET /api/base/check-orders
 * 外部クーロンサービス（cron-job.org など）から呼ばれる注文チェックエンドポイント。
 * vercel.json の crons 配列には入れないこと（入れると外部リクエストが404になる）。
 *
...
async function pushLine(lineUserId: string, text: string): Promise<void> {
  ...
  if (!res.ok) console.error('[check-orders] LINE push失敗', res.status, await res.text())
}
...
export async function GET(request: NextRequest) {
  ...
  const params = new URLSearchParams({ start_ordered: startDate, limit: '100', offset: '0' })
  ...
  for (const order of orders) {
    ...
    if (doThankyou && lineUserId) {
      const text =
        ...
        `${buildProductLines(ordered_products ?? [])}\n\n` +
        `丁寧に仕上げてお届けいたします。\n` +
        `発送時にはまたLINEでお知らせします。\n\n` +
        `どうぞよろしくお願いいたします。`
      await pushLine(lineUserId, text)
      await supabase.from('messages').insert({
        line_user_id: lineUserId, direction: 'outbound', message_type: 'text',
  ...
  console.log(`[check-orders] 完了 thankyou=${thankyouSent} shipping=${shippingSent}`)
