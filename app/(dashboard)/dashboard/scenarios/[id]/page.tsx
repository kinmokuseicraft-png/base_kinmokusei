...
export default function ScenarioEditPage() {
  ...
  useEffect(() => {
    if (isNew) return;
    fetch(`/api/scenarios/${id}`, { cache: "no-store" })
      .then((r) => r.json())
      .then((d: { scenario?: { name: string; description: string | null; trigger_type: TriggerType; trigger_value: string | null; is_active: boolean }; steps?: { id: string; step_order: number; delay_hours: number; message_content: { text?: string } }[] }) => {
        if (d.scenario) {
          setName(d.scenario.name);
          setDescription(d.scenario.description ?? "");
          setTriggerType(d.scenario.trigger_type);
          setTriggerValue(d.scenario.trigger_value ?? "");
          setIsActive(d.scenario.is_active);
          setSteps((d.steps ?? []).map((s, i) => ({
            id: s.id,
            step_order: i,
            delay_hours: s.delay_hours,
            text: s.message_content?.text ?? "",
          })));
        }
      })
  ...
  async function handleSave() {
    ...
    const res = isNew
      ...
      : await fetch(`/api/scenarios/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", marginBottom: "var(--space-5)" }}>
        <Link
          ...
          style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 34, height: 34, border: "1px solid var(--color-border)", color: "var(--color-text-muted)", textDecoration: "none" }}
        >
          <ArrowLeft size={16} />
        </Link>
        <h1 style={{ fontSize: "1.3rem", fontWeight: 700 }}>
          {isNew ? "新しいシナリオ" : "シナリオを編集"}
        </h1>
      </div>
      ...
      <section style={{ marginBottom: "var(--space-4)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-3)" }}>
          ...
          <button
            onClick={addStep}
            style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.8rem", padding: "5px 12px", border: "1px solid var(--color-border)", background: "var(--color-surface)", color: "var(--color-text-muted)", cursor: "pointer" }}
          >
              ...
              <div key={i} style={{ padding: "var(--space-4)", background: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-3)" }}>
                  ...
                  <button
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 4, fontSize: "0.75rem", color: "#e74c3c", border: "none", background: "transparent", cursor: "pointer" }}
                  >
                    <Trash2 size={13} /> 削除
                  </button>
                </div>
                  ...
                  {i === 0 ? (
                    ...
                    <>
                      ...
                      <input
                        ...
                        style={{ width: 64, padding: "3px 8px", background: "var(--color-bg)", border: "1px solid var(--color-border)", color: "var(--color-text)", fontSize: "0.82rem", textAlign: "center" }}
                      />
                      <span>時間後に送信</span>
                    </>
                  )}
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder={`Step ${i + 1} のメッセージを入力...`}
                  style={{ width: "100%", padding: "0.5rem 0.6rem", background: "var(--color-bg)", border: "1px solid var(--color-border)", color: "var(--color-text)", fontSize: "0.85rem", lineHeight: 1.7, resize: "vertical", boxSizing: "border-box" }}
                />
              </div>
            ))}
          </div>
      ...
      {error && (
        <p style={{ fontSize: "0.82rem", color: "#e74c3c", background: "#2a1010", border: "1px solid #5a2020", padding: "0.5rem 0.75rem", marginBottom: "var(--space-3)" }}>
          {error}
        </p>
      )}
      ...
      <div style={{ display: "flex", gap: "var(--space-3)" }}>
        <Link
          href="/dashboard/scenarios"
          style={{ flex: 1, padding: "0.65rem", textAlign: "center", border: "1px solid var(--color-border)", color: "var(--color-text-muted)", fontSize: "0.88rem", textDecoration: "none" }}
        >
          キャンセル
        </Link>
        <button
          onClick={handleSave}
          disabled={saving}
          style={{ flex: 1, padding: "0.65rem", background: saving ? "var(--color-border)" : "var(--color-line)", color: "#fff", fontWeight: 700, fontSize: "0.88rem", border: "none", cursor: "pointer" }}
        >
          {saving ? "保存中..." : "保存する"}
        </button>
      </div>
